...
- Frontend: `news/[slug].astro` looks up a published `gil` localization by the article's `documentId`; if found, a zero-JS English/Kiribati pill toggle (query-param links, `?lang=gil`) renders under the date and swaps title/body (photo + date shared). No translation → no toggle; stray `?lang=gil` falls back to English.
- Editors: Content Manager locale switcher (top right of a News entry) → "Kiribati (Gilbertese) (gil)" → fill title/body → Publish. Toggle appears per-article once its translation is published.
- Verified locally 2026-08-17: test translation of the Starlink article seeded via `strapi console` (placeholder Gilbertese, local dev DB only); browser-verified toggle both directions, no-toggle + fallback states, missing-translation findOne 404s. `tsc --noEmit` (backend), `astro check` (0 errors) + `npm run build` pass clean. Merged + pushed 2026-08-17 (`72e7741`); Strapi Cloud + Vercel auto-deploy.
- Post-deploy: live articles need their Kiribati translations entered + published in Strapi Cloud before the toggle shows.
